...
Production-oriented Taiwan stock auto trading framework using Shioaji API.  這是一個專門為台灣股市打造的自動交易框架，核心用 Shioaji API。它能幫你接行情、跑策略、控風險，最後自動下單，整套流程都能在生產環境穩定運行。
# Shioaji Stable Lab

Production-oriented trading framework for Taiwan stock market using Shioaji API.
